...
def register_backbone(name: str, *, aliases=(), experimental=False):
    def decorator(cls):
        cls._is_experimental = experimental
        backbone_registry[name] = cls
        for alias in aliases:
            backbone_registry[alias] = cls
        return cls
    return decorator

def list_backbones(verbose=False):
    for name, cls in backbone_registry.items():
        tag = "[EXPERIMENTAL]" if getattr(cls, "_is_experimental", False) else ""
        doc = cls.__doc__ or ""
        desc = doc.splitlines()[0].strip() if doc else ""
        print(f"- {name} {tag} :: {desc}" if verbose else f"- {name} {tag}")


class BackboneBase(nn.Module):
    def __init__(self, backbone: nn.Module, train_backbone: bool, num_channels: int, return_interm_layers: bool, check_featuremap: bool):
        super().__init__()
